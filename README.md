...
| **Werkbank (Web-IDE)** | Pro-User code-server (VS Code im Browser) unter `/workbench`; integriertes Terminal, Git/GitLab, SSH zu `*.example.com`; **vorinstallierte KI-Agenten-Extensions** (Claude Code + OpenAI Codex); Ansible-Playbooks editierbar (geteiltes `playbooks/`-Verzeichnis, sofort in AWX sichtbar); Hermes-Analyse als Markdown übergeben |
...
- **SSH** to `*.example.com` is wired up from the host `~/.ssh` mount (marvin key)
  by the entrypoint.
