    ...
    <div class="feed-page">
      ...
      <div class="glog-bar">
        ...
        <input class="glog-input" type="text" [(ngModel)]="searchQuery"
               ...
               placeholder='Search all sources — e.g. metadata.host:"host.example.com" AND severity:high'>
        ...
        @if (searchActive()) {
          <button mat-icon-button class="glog-clear" (click)="clearFeedSearch()" matTooltip="Reset search">
      ...
      <div class="feed-column">
        ...
        @if (searchActive()) {
          <div class="search-active-banner">
            ...
            <span>Search across all sources: <b>{{ activeSearchQuery() }}</b> · {{ searchRangeLabel() }} · {{ items().length }} hits</span>
            <span class="glog-spacer"></span>
            <button mat-icon-button (click)="clearFeedSearch()" matTooltip="Reset search"><mat-icon>close</mat-icon></button>
          </div>
        }
        ...
        @if (loading() || searching()) {
          <div class="refresh-indicator">
            ...
            <span>{{ searching() ? 'Searching…' : 'Refreshing…' }}</span>
          </div>
        }
  ...
  runFeedSearch() {
    ...
    this.http.get<FeedItem[]>(`${environment.apiUrl}/feed/`, { params }).subscribe({
      ...
      error: () => {
        ...
        this.items.set([]);
        this.snackBar.open('Search failed', 'OK', { duration: 5000 });
      },
    });
  }
