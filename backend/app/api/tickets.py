...
async def ticket_draft(
    ...
    if body.feed_external_id:
        ...
        # Load AI analysis comments — these contain the evidence block collected
        # by the feed analysis pipeline and are the primary proof source.
        try:
        ...
        raise HTTPException(400, "feed_external_id or transcript required")
...
async def ticket_create(
    ...
    if not result.get("ok"):
        raise HTTPException(502, f"Ticket could not be created: {result.get('error', '')}")
